...
function handles = graphs_emggait(handles)
% GRAPHS_EMGGAIT Initialization function to create GUI of axes
...
signal = handles.reader.signal_f;
...
amp = handles.processed.amp_max;

% creates the panel for emg-gait analysis processing
tic
for n = 1:n_channels
    ...
    [~, ~, ~] = plot_emggait(handles.haxes(1, n), [pmax_I{1} amp{1}], signal(:,n), xs,...
...
% emg-gait application does not have button down function
set(ax, 'LooseInset', loose_inset, 'FontSize', 7, 'NextPlot', 'add');
...
[~, ~, ~] = plot_emggait(ax, signal, xs, [pmax_I amp], [emg_lat emg_end]);
